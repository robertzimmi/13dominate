...
player_pairings AS (
  ...
  WHERE p."Player1Name" = %(player_name)s OR p."Player2Name" = %(player_name)s
),
...
def api_player_chart_data():
    ...
    if show_all:
        query = """
            SELECT
                CAST(e.data AS date) AS data,
                SUM(CAST(s."Wins" AS INTEGER)) AS wins
            FROM standings s
            JOIN eventos e ON s.event_id = e.id
            WHERE s."Name" = %s
            GROUP BY data
        ...
        query = """
            ...
            FROM standings s
            JOIN eventos e ON s.event_id = e.id
            WHERE s."Name" = %s AND CAST(e.data AS date) >= CURRENT_DATE - INTERVAL '30 days'
            GROUP BY data
            ORDER BY data
        """
...
def fetch_all_heroes_for_player(player_name):
    ...
    WITH hero_data AS (
        ...
        JOIN heroes h ON s."PlayerID" = h."PlayerID"
            AND h.event_id = s.event_id
        JOIN eventos e ON s.event_id = e.id AND h.event_id = e.id
    ...
    hero_rounds AS (
        ...
        JOIN heroes h ON (
            ...
            AND h.event_id = p.event_id
        )
        JOIN eventos e ON p.event_id = e.id AND h.event_id = e.id
        WHERE h."PlayerName" = %s
        GROUP BY h."Hero"
    )
    SELECT
...
def fetch_current_month_heroes_for_player(player_name):
    ...
    hero_data AS (
        ...
        JOIN heroes h ON s."PlayerID" = h."PlayerID"
            AND h.event_id = s.event_id
        JOIN eventos e ON s.event_id = e.id AND h.event_id = e.id
        JOIN date_filter df ON date_trunc('month', e.data) = df.this_month
        WHERE h."PlayerName" = %s
    ),
    hero_aggregates AS (
        SELECT
